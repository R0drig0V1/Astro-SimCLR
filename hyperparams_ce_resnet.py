...
def ce_trainer(config_hyper, max_epochs=100, gpus=1):
    ...
    model = Supervised_Cross_Entropy_Resnet(
...
def ce_tune(num_samples, max_epochs, cpus_per_trial=1, gpus_per_trial=1):
    ...
    config_hyper = {
        ...
        "augmentation": tune.choice(['rotation_v3'])
    }
    ...
    # Dataframe is saved
    df.to_csv('results/hyperparameter_tuning_ce_resnet.csv', float_format='%.6f')

    # Summary of hyperparameter tuning
    df_summary = summary(df, metric="accuracy")
    df_summary.to_csv('results/summary_tuning_ce_resnet.csv', float_format='%.6f')
...
trials = ["6a9731ae", "7ac1c73c", "cf11ac9a"]
checkpoints = [f"/home/rvidal/hyperparameter_tuning/CE_Resnet/id_{trial}/checkpoint.ckpt" for trial in trials]

# Train for different initial conditions
for checkpoint_path in checkpoints:

    # Load weights
    sce = Supervised_Cross_Entropy_Resnet.load_from_checkpoint(checkpoint_path)
...
title = f'Confusion matrix Resnet18\n Accuracy Validation:{acc_mean_val:.3f}$\pm${acc_std_val:.3f}'
file = 'figures/confusion_matrix_CE_Resnet18_Validation.png'
...
file = 'figures/confusion_matrix_CE_Resnet18_Test.png'
...
# Dataframe with results
df = pd.read_csv('results/hyperparameter_tuning_ce_resnet.csv', index_col=0)

# Summary of hyperparameter tuning
df_summary = pd.read_csv('results/summary_tuning_ce_resnet.csv', index_col=0)
...
shutil.copy(hparams_path, "results/hparams_best_ce_resnet.yaml")
