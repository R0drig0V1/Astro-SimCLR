...
from utils.training import SimCLR_encoder_classifier_2_datasets
...
def training_simclr(hparams, rep):
    ...
    checkpoint_callback = ModelCheckpoint_V2(
        monitor="loss_train_enc",
        dirpath=os.path.join(config.model_path, "SimCLR_300_resnet_18_27_astro2"),
    ...
    tb_logger = TensorBoardLogger(
        save_dir='tb_logs',
        name='simclr_300_resnet_18_27_astro2',
        version=hparams.augmentation
    )

    # Initialize pytorch_lightning module
    model = SimCLR_encoder_classifier_2_datasets(
        **hparams,
    ...
    # Loads weights
    model = SimCLR_encoder_classifier_2_datasets.load_from_checkpoint(path)
...
Accuracy Validation:{acc_mean_val:.3f}$\pm${acc_std_val:.3f}"""
        file = f"figures/confusion_matrix_SimCLR_300_{label_encoder[hparams.encoder_name]}_27-Validation-{label_features[with_features][1]}-{label_aug[augmentation][1]}.png"
...
Accuracy Test:{acc_mean_test:.3f}$\pm${acc_std_test:.3f}"""
        file = f"figures/confusion_matrix_SimCLR_300_{label_encoder[hparams.encoder_name]}_27-Test-{label_features[with_features][1]}-{label_aug[augmentation][1]}.png"
