...
def training_ce(hparams, name_checkpoint, name_tb, data_path):
    ...
    checkpoint_callback = ModelCheckpoint_V2(
        monitor="accuracy_val",
        dirpath=os.path.join(config.model_path, "CE_resnet18_red"),
    ...
    tb_logger = TensorBoardLogger(
        save_dir='tb_logs',
        name='ce_resnet18_red',
        version=name_tb
    )

    # Initialize model
    model = Supervised_Cross_Entropy_Resnet(**hparams, data_path=data_path, resnet_model='resnet18')
...
for augmentation in tqdm(label_aug.keys(), desc='Augmentations', unit= "aug"):
    for with_features in tqdm(label_features.keys(), leave=False, desc='Features', unit="feat"):
        for p in tqdm(paths.keys(), desc='dataset\'s fraction', unit= "p"):
            ...
            # ---------------------------------
            title = f"""Confusion matrix ResNet18 classifier (labels {p}%)
({label_features[with_features][0]}, {label_aug[augmentation][0]})
Accuracy Validation:{acc_mean_val:.3f}$\pm${acc_std_val:.3f}"""
            file = f"figures/confusion_matrix_CE_Resnet18-frac_{p}-Validation-{label_features[with_features][1]}-{label_aug[augmentation][1]}.png"
...
Accuracy Test:{acc_mean_test:.3f}$\pm${acc_std_test:.3f}"""
            file = f"figures/confusion_matrix_CE_Resnet18-frac_{p}-Test-{label_features[with_features][1]}-{label_aug[augmentation][1]}.png"
