...
def training_simclr_classifier(simclr_model, augmentation, name_checkpoint, name_tb, with_features, data_path):
    ...
    checkpoint_callback = ModelCheckpoint_V2(
        ...
        dirpath=os.path.join(config.model_path, "Linear_classifier_stamps_2"),
    ...
    logger = TensorBoardLogger(
        save_dir='tb_logs',
        name='linear_classifier_stamps_2',
        version=name_tb
    )
    ...
    for with_features in tqdm(label_features.keys(), leave=False, desc='Features', unit="feat"):
        for p in tqdm(paths.keys(), desc='dataset\'s fraction', unit= "p"):
            ...
            for simclr_aug in simclr_augs:
                ...
                folders = [f"/home/rvidal/weights/SimCLR_300_stamps_20/checkpoint_{simclr_aug}_{i+1}.ckpt" for i in range(3)]
...
Accuracy Validation:{acc_mean_val:.3f}$\pm${acc_std_val:.3f}"""
                file = f"figures/confusion_matrix_linear_classifier_300_frac_{p}-Validation-{label_features[with_features][1]}-{label_aug2[simclr.augmentation][1]}-{label_aug2[augmentation][1]}-{label_encoder[encoder_name]}_2.png"
...
Accuracy Test:{acc_mean_test:.3f}$\pm${acc_std_test:.3f}"""
                file = f"figures/confusion_matrix_linear_classifier_300_frac_{p}-Test-{label_features[with_features][1]}-{label_aug2[simclr.augmentation][1]}-{label_aug2[augmentation][1]}-{label_encoder[encoder_name]}_2.png"
