...
def training_fine_simclr(simclr_model, augmentation, name_checkpoint, name_tb, data_path):
    ...
    checkpoint_callback = ModelCheckpoint_V2(
        ...
        dirpath=os.path.join(config.model_path, "Fine_tuning_300_stamps_20"),
...
trials = ["checkpoint_astro8_1.ckpt", "checkpoint_astro8_2.ckpt", "checkpoint_astro8_3.ckpt"]#, "checkpoint_astro2_4.ckpt", "checkpoint_astro2_5.ckpt"]
folders = [f"/home/rvidal/weights/SimCLR_300_stamps_20/{trial}" for trial in trials]
...
Accuracy Validation:{acc_mean_val:.3f}$\pm${acc_std_val:.3f}"""
            file = f"figures/confusion_matrix_fine_tuning-300-frac_{p}-Validation-{label_features[with_features][1]}-{label_aug2[simclr.augmentation][1]}-{label_aug[augmentation][1]}-{label_encoder[encoder_name]}_2.png"
...
Accuracy Test:{acc_mean_test:.3f}$\pm${acc_std_test:.3f}"""
            file = f"figures/confusion_matrix_fine_tuning-300-frac_{p}-Test-{label_features[with_features][1]}-{label_aug2[simclr.augmentation][1]}-{label_aug[augmentation][1]}-{label_encoder[encoder_name]}_2.png"
